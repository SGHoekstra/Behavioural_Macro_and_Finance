...
h5open(dir * "/" * model_type * "/" * distribution_type * "/forecast_abmx_uf.h5", "w") do file
    write(file, "forecast", forecast)
end

rmse_abmx = dropdims(100 * sqrt.(nanmean((forecast - actual).^2,1)), dims=1)
bias_abmx = dropdims(nanmean(forecast - actual, 1), dims=1)
error_abmx = forecast - actual

file_path = dir * "/forecast_arx.h5"
...
input_data = - round.(100 * (rmse_abmx .- rmse_arx) ./ rmse_arx, digits=1)
input_data_S = fill("", size(input_data))

for j in 1:length(horizons)
    h = horizons[j]
    for l in 1:number_variables
        dm_error_abmx = view(error_abmx, :, j, l)[map(!,isnan.(view(error_abmx, :, j, l)))]
        dm_error_arx = view(error_arx, :, j, l)[map(!,isnan.(view(error_arx, :, j, l)))]
        _, p_value = BIT.dmtest_modified(dm_error_abmx,dm_error_arx, h)
...
open(dir * "/" * model_type * "/" * distribution_type * "/rmse_abmx_uf.tex", "w") do fid
    for line in latex
        write(fid, line * "\n")
    end
end
...
input_data = round.(bias_abmx, digits=4)
input_data_S = fill("", size(input_data))
...
for j in 1:length(horizons)
    ...
    h = horizons[j]
    for l in 1:number_variables
        mz_forecast = (view(error_abmx, :, j, l) + view(actual, :, j, l))[map(!,isnan.(view(error_abmx, :, j, l) + view(actual, :, j, l)))]
...
open(dir * "/" * model_type * "/" * distribution_type * "/bias_abmx_uf.tex", "w") do fid
    for line in latex
        write(fid, line * "\n")
    end
end
