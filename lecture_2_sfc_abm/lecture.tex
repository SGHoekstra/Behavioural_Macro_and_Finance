...
\begin{frame}{What are agent-based model typical elements again?}
    \begin{columns}
...
\begin{frame}{From Qualitative to Quantitative ABMs}
    \begin{itemize}
        \item Recall the \cite{assenza_emergent_2015} model from last lecture: a qualitative macro ABM that generates emergent business cycles.
        \item Today we move to \textbf{quantitative} ABMs: the \cite{poledna_economic_2019} model calibrated to real data.
        \item We then discuss advanced calibration techniques to identify the remaining free parameters.
    \end{itemize}
\end{frame}
...
\begin{frame}{Fiscal and monetary closure}
    ...
    \begin{itemize}\footnotesize
        ...
        \item \textbf{Policy rate} $\bar{r}_t$: generalised Taylor rule reacting to \emph{euro-area} inflation and growth (next slide).
    \end{itemize}
    \end{columns}
    \vspace{0.4em}
    \footnotesize
...
\begin{frame}{What drives fluctuations? Five exogenous AR(1) processes}
    \begin{itemize}
        \item Unlike \cite{assenza_emergent_2015}, where fluctuations are entirely endogenous, the Poledna ABM is driven by \textbf{five exogenous AR(1) processes} estimated from data:
        \begin{enumerate}\footnotesize
            \item Export demand $C^{\mathrm{E}}_t$
            \item Import supply $Y^{\mathrm{I}}_t$
            \item Government consumption $C^{\mathrm{G}}_t$
            \item Euro-area GDP growth $\gamma^{\mathrm{EA}}_t$
            \item Euro-area inflation $\pi^{\mathrm{EA}}_t$
        \end{enumerate}
        \item The shocks are correlated via a covariance matrix $\mathbf{C}$ estimated from national accounts.
        \item Euro-area growth and inflation feed directly into the Taylor rule $\Rightarrow$ monetary policy reacts to the euro-area cycle, not just Austrian conditions (small open economy).
        \item Endogenous fluctuations are \emph{amplified} by these shocks but not \emph{caused} by them; bankruptcy cascades generate their own crises (see next).
    \end{itemize}
\end{frame}

\begin{frame}{Endogenous crises: the bankruptcy cascade}
    \begin{itemize}
        \item A firm becomes insolvent when \textbf{both}: cash-flow negative ($D_{i,t} < 0$) \emph{and} balance-sheet negative ($E_{i,t} < 0$).
        \item Insolvency triggers a \textbf{cascade} through three channels:
        \begin{itemize}\footnotesize
            \item \textbf{Labour}: workers at the failing firm are laid off $\to$ household income drops $\to$ consumption falls.
            \item \textbf{Supply chain}: upstream suppliers lose a buyer $\to$ inventory build-up $\to$ their profits fall.
            \item \textbf{Credit}: the bank writes off bad debt $\to$ $E_{k,t}$ drops $\to$ the minimum capital requirement binds $\to$ credit to \emph{other} firms is rationed.
        \end{itemize}
        \item Each channel is non-linear and \textbf{path-dependent}: the economy does not return to the BLE it came from.
        \item Multiple bankruptcies amplify the initial shock into a \textbf{medium-term recession} without any further exogenous disturbance.
        \item This is the mechanism behind ``endogenous crises'', the feature that distinguishes the ABM from linearised DSGE, where balance-sheet constraints do not bind.
    \end{itemize}
\end{frame}
...
\begin{frame}{Data Sources}
    \begin{itemize}
        \item CBS Register data and Eurostat data
...
\begin{frame}{The Likelihood Problem in Agent-Based Models}
\label{SBI-motivation}

\textbf{Goal:} Bayesian inference for ABM parameters
$$p(\theta|x_{\text{obs}}) = \frac{p(x_{\text{obs}}|\theta)p(\theta)}{p(x_{\text{obs}})}$$

\textbf{The Problem:} Likelihood $p(x|\theta)$ is intractable. We can simulate $x \sim \text{ABM}(\theta)$, but cannot evaluate $p(x|\theta)$

\textbf{Why standard Bayesian methods fail:}
\begin{itemize}
    \item \textit{MCMC:} Acceptance ratio $\alpha = \min\left(1, \frac{p(x|\theta')p(\theta')}{p(x|\theta)p(\theta)}\cdot\frac{q(\theta|\theta')}{q(\theta'|\theta)}\right)$ requires likelihood evaluation at every step
    
    \item \textit{Variational Inference:} Evidence Lower Bound (ELBO) $= \mathbb{E}_{q_\phi}\left[\log p(x|\theta)\right] - D_{KL}(q_\phi \| p(\theta))$ requires likelihood in expectation
    
    \item \textit{Approximate Bayesian Computation:} Works, but acceptance rate suffers from the curse of dimensionality.
\end{itemize}

\end{frame}

\begin{frame}{Neural Posterior Estimation \citep{dyer_black-box_2024}}
    \begin{itemize}
        \item \textbf{Problem:} Need posterior $p(\theta|x)$ but the likelihood $p(x|\theta)$ is intractable for ABMs.
        \item \textbf{Solution: Neural Posterior Estimation:}
        \begin{itemize}
            \item Generate training data $\{x_i,\theta_i\}_{i=1}^{N_{\text{sims}}}$ by sampling $\theta_i \sim \Phi$ (prior) and simulating $x_i \sim \text{ABM}(\theta_i)$.
            \item Train neural network $q_\phi(\theta|x)$ to approximate posterior:
            \[ \hat{\phi}^{(r)} = \arg\max_\phi \sum_{i=1}^{N_{\text{sims}}} \log q_\phi(\theta_i|x_i) \]
            \item Refine with sequential rounds using current posterior as new prior:
            \[ \theta_i^{(r+1)} \sim q_{\hat{\phi}^{(r)}}(\theta|x_{\text{obs}}) \]
            \item For any observed data $x_{\text{obs}}$, directly sample from final posterior:
            \[ \theta \sim q_{\hat{\phi}^{\text{final}}}(\theta|x_{\text{obs}}) \]
        \end{itemize}
        \item \textbf{Key advantage:} Overcomes intractable likelihood by learning the posterior directly, bypassing likelihood evaluation entirely and enabling full posterior uncertainty quantification.
    \end{itemize}
\end{frame}


\begin{frame}{Sequential Neural Posterior Estimation (SNPE)}
    \footnotesize
    \textbf{Motivation:} a single round of NPE wastes simulations on regions of $\boldsymbol{\Theta}$ that turn out to have low posterior density given the observed $\mathbf{y}$. SNPE narrows the search iteratively by re-using the current posterior estimate as the proposal distribution for the next batch of simulations, concentrating compute where it matters.
    \vspace{0.2em}

    \begin{algorithm}[H]
    \caption{Sequential Neural Posterior Estimation (SNPE)}
    \begin{algorithmic}[1]
    \State Initialise proposal $\tilde{p}_0(\boldsymbol{\theta}) = p(\boldsymbol{\theta})$; dataset $\mathcal{D} = \emptyset$
    \For{$m = 0, \ldots, M-1$ rounds}
        \State Sample $\boldsymbol{\theta}^{(n)} \sim \tilde{p}_m(\boldsymbol{\theta})$; simulate $\mathbf{x}^{(n)} \sim p(\mathbf{x}|\boldsymbol{\theta}^{(n)})$; append to $\mathcal{D}$
        \State Train the conditional flow $q_\phi(\boldsymbol{\theta}|\mathbf{x})$ with the \textbf{importance-reweighted} loss
        \[
        \mathcal{L}(\phi) = -\!\!\!\sum_{(\mathbf{x},\boldsymbol{\theta})\in\mathcal{D}}\!\!\log\!\left( q_\phi(\boldsymbol{\theta}|\mathbf{x}) \cdot \frac{\tilde{p}_m(\boldsymbol{\theta})}{p(\boldsymbol{\theta})} \cdot \frac{1}{Z(\mathbf{x},\phi)} \right)
        \]
        \State Update proposal: $\tilde{p}_{m+1}(\boldsymbol{\theta}) := q_\phi(\mathbf{y},\boldsymbol{\theta})$
    \EndFor
    \end{algorithmic}
    \end{algorithm}

    \footnotesize
    \textbf{The importance weight $\tilde{p}_m/p$ is essential:} after round 1, parameters are no longer drawn from the prior $p(\boldsymbol{\theta})$ but from the (narrower) proposal $\tilde{p}_m(\boldsymbol{\theta})$. The reweighting corrects for this shift; without it, SNPE converges to the wrong distribution.

    \textbf{Trade-off:} SNPE is more simulation-efficient \emph{for a specific} $\mathbf{y}$ than amortised NPE, but it \textbf{loses amortisation}: a new observation $\mathbf{y}'$ requires retraining the proposal sequence from scratch.
\end{frame}

\begin{frame}{Results: decisive improvement over KDE}
    \small
    \textbf{Benchmark 1: \cite{brock_heterogeneous_1998}}: four heuristic belief types with intensity of choice $\beta$, parameter $\boldsymbol{\theta}=(g_2,b_2,g_3,b_3)$.
    \vspace{0.2em}
    \begin{center}
    \begin{tabular}{lcc}
    \toprule
    ...
    \midrule
    Wasserstein (param set 1) & 0.690 & \textbf{0.336} \\
    MMD                       & 1.015 & \textbf{0.552} \\
    Simulation budget         & $1.5\times 10^5$ & $10^4$ \\
    \bottomrule
    \end{tabular}
    \end{center}
    \vspace{0.4em}

    \textbf{Benchmark 2: Multivariate Geometric Brownian Motion}: $d=3$ drift/volatility inference.
    \begin{center}
    \begin{tabular}{lcc}
    \toprule
    & \textbf{KDE} & \textbf{NPE} \\
    \midrule
    Wasserstein       & 0.364 & \textbf{0.099} \\
    MMD               & 0.137 & \textbf{0.005} \\
    Simulation budget & $10^6$ & $10^3$ \\
    \bottomrule
    \end{tabular}
    \end{center}
    \vspace{0.3em}
    \normalsize
    \textbf{10--1000$\times$ reduction in simulation budget, \emph{and} better posterior recovery.}
\end{frame}

\begin{frame}{Visual results: Brock--Hommes posteriors}
    \begin{figure}
        \centering
        \includegraphics[width=0.5\linewidth]{figs/posterior_SPNE.png}
        \caption{\scriptsize SNPE with learned summary statistics on the \cite{brock_heterogeneous_1998} asset-pricing model. Marginal posteriors on the diagonal, bivariate joints above; red lines show the true posterior mean. Source: \cite{dyer_black-box_2024}, Fig.~3d. Code: \texttt{joelnmdyer/sbi4abm} (a wrapper around the Mackelab \texttt{sbi} package).}
    \end{figure}
\end{frame}
...
\begin{itemize}
    \item We have discussed the \textbf{qualitative} \cite{assenza_emergent_2015} model in detail.
