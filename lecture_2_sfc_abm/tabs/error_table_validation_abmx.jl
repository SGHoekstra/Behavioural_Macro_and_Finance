...
model = load("./src/utils/data/" * country * "/" * model_type* "/" * distribution_type *"/predictions_abmx/" * string(year(BIT.num2date(quarter_num))) * "Q" * string(Dates.quarterofyear(BIT.num2date(quarter_num))) *".jld2","model_dict");
number_of_seeds = size(model["real_gdp_quarterly"],2)

for i in 1:number_quarters
    quarter_num = quarters_num[i]

    global model = load("./src/utils/data/"* country * "/" * model_type* "/" * distribution_type *"/predictions_abmx/" * string(year(BIT.num2date(quarter_num))) * "Q" * string(Dates.quarterofyear(BIT.num2date(quarter_num))) *".jld2","model_dict");
...
h5open(dir * "/" * model_type* "/" * distribution_type *"/forecast_validation_abmx.h5", "w") do file
    write(file, "forecast", forecast)
end

rmse_validation_abmx = dropdims(100 * sqrt.(nanmean((forecast - actual).^2,1)), dims=1)
bias_validation_abmx = dropdims(nanmean(forecast - actual, 1), dims=1)
...
input_data = - round.(100 * (rmse_validation_abmx .- rmse_validation_varx) ./ rmse_validation_varx, digits=1)
input_data_S = fill("", size(input_data))
for j in 1:length(horizons)
    h = horizons[j]
    for l in 1:number_variables
        dm_error_validation_abmx = view(error_validation_abmx, :, j, l)[map(!,isnan.(view(error_validation_abmx, :, j, l)))]
        dm_error_validation_varx = view(error_validation_varx, :, j, l)[map(!,isnan.(view(error_validation_varx, :, j, l)))]
        _, p_value = BIT.dmtest_modified(dm_error_validation_varx,dm_error_validation_abmx, h)
...
open(dir * "/" * model_type* "/" * distribution_type *"/rmse_validation_abmx.tex", "w") do fid
    for line in latex
        write(fid, line * "\n")
    end
end
...
input_data = round.(bias_validation_abmx, digits=4)
input_data_S = fill("", size(input_data))
...
for j in 1:length(horizons)
    ...
    h = horizons[j]
    for l in 1:number_variables
        mz_forecast = (view(error_validation_abmx, :, j, l) + view(actual, :, j, l))[map(!,isnan.(view(error_validation_abmx, :, j, l) + view(actual, :, j, l)))]
...
open(dir * "/" * model_type* "/" * distribution_type *"/bias_validation_abmx.tex", "w") do fid
    for line in latex
        write(fid, line * "\n")
    end
end
