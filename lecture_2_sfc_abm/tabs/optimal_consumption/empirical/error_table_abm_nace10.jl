...
model = load("./src/utils/data/" * country * "/" * model_type* "/" * distribution_type *"/predictions_abm_uf/" * string(year(BIT.num2date(quarter_num))) * "Q" * string(Dates.quarterofyear(BIT.num2date(quarter_num))) *".jld2","model_dict");
number_of_seeds = size(model["real_gdp_quarterly"],2)

for i in 1:number_quarters
    quarter_num = quarters_num[i]

    global model = load("./src/utils/data/" * country * "/" * model_type* "/" * distribution_type *"/predictions_abm_uf/" * string(year(BIT.num2date(quarter_num))) * "Q" * string(Dates.quarterofyear(BIT.num2date(quarter_num))) *".jld2","model_dict");
...
file_path = dir * "/forecast_ar_nace10.h5"
...
input_data = - round.(100 * (rmse_abm_nace10 .- rmse_ar_nace10) ./ rmse_ar_nace10, digits=1)
...
for j in 1:length(horizons)
    ...
    for l in 1:number_variables
        ...
        _, p_value = BIT.dmtest_modified(dm_error_abm_nace10,dm_error_ar_nace10, h)
...
open(dir * "/" * model_type* "/" * distribution_type *"/rmse_abm_nace10.tex", "w") do fid
    for line in latex
        write(fid, line * "\n")
    end
end
...
input_data = round.(bias_abm_nace10, digits=4)
input_data_S = fill("", size(input_data))
...
for j in 1:length(horizons)
    ...
    h = horizons[j]
    for l in 1:number_variables
        mz_forecast = (view(error_abm_nace10, :, j, l) + view(actual, :, j, l))[map(!,isnan.(view(error_abm_nace10, :, j, l) + view(actual, :, j, l)))]
...
open(dir * "/" * model_type* "/" * distribution_type *"/bias_abm_nace10.tex", "w") do fid
    for line in latex
        write(fid, line * "\n")
    end
end
