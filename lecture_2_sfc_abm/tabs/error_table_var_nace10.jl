...
for k = 1:3
    ...
    for i in 1:number_quarters
        ...
        for j in 1:number_horizons
            ...
            Y0 = log.(data["nominal_nace10_gva_quarterly"][repeat(data["quarters_num"] .<= quarter_num,1,10)])
    ...
    if k == 1
        h5open(dir * "/forecast_var_nace10.h5", "w") do file
            write(file, "forecast", forecast)
        end
        rmse_var_nace10 = dropdims(100 * sqrt.(nanmean((forecast - actual).^2,1)), dims=1)
        bias_var_nace10 = dropdims(nanmean(forecast - actual, 1), dims=1)
        error_var_nace10 = forecast - actual
    else
        h5open(dir * "/forecast_var_nace10_$(k).h5", "w") do file
            write(file, "forecast", forecast)
        end
        rmse_var_nace10_k = dropdims(100 * sqrt.(nanmean((forecast - actual).^2,1)), dims=1)
        bias_var_nace10_k = dropdims(nanmean(forecast - actual, 1), dims=1)
        error_var_nace10_k = forecast - actual

        forecast = h5read(dir * "/forecast_var_nace10.h5","forecast")
        rmse_var_nace10 = dropdims(100 * sqrt.(nanmean((forecast - actual).^2,1)), dims=1)
        error_var_nace10 = forecast - actual
    end
    ...
    if k == 1
        global input_data = round.(rmse_var_nace10, digits=2)
        global input_data_S = string.(input_data)
    else
        input_data = - round.(100 * (rmse_var_nace10 .- rmse_var_nace10_k) ./ rmse_var_nace10, digits=1)
        input_data_S = fill("", size(input_data))
        for j in 1:length(horizons)
            h = horizons[j]
            for l in 1:number_variables
                dm_error_var_nace10_k = view(error_var_nace10_k, :, j, l)[map(!,isnan.(view(error_var_nace10_k, :, j, l)))]
                dm_error_var_nace10 = view(error_var_nace10, :, j, l)[map(!,isnan.(view(error_var_nace10, :, j, l)))]
                _, p_value = BIT.dmtest_modified(dm_error_var_nace10,dm_error_var_nace10_k, h)
    ...
    if k == 1
        open(dir * "/rmse_var_nace10.tex", "w") do fid
            for line in latex
                write(fid, line * "\n")
            end
        end
    else
        open(dir * "/rmse_var_nace10_$(k).tex", "w") do fid
            for line in latex
                write(fid, line * "\n")
            end
        end
    ...
    if k == 1
        input_data = round.(bias_var_nace10, digits=4)
        input_data_S = fill("", size(input_data))
        ...
        for j in 1:length(horizons)
            ...
            h = horizons[j]
            for l in 1:number_variables
                mz_forecast = (view(error_var_nace10, :, j, l) + view(actual, :, j, l))[map(!,isnan.(view(error_var_nace10, :, j, l) + view(actual, :, j, l)))]
        ...
        open(dir * "/bias_var_nace10.tex", "w") do fid
            for line in latex
                write(fid, line * "\n")
            end
        end
